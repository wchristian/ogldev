...
has $_ => ( is => 'rw' )
  for
  qw( WVPLocation samplerLocation WorldMatrixLocation eyeWorldPosLocation matSpecularIntensityLocation matSpecularPowerLocation numPointLightsLocation );
has dirLightLocation    => ( is => 'rw', default => sub { {} } );
has pointLightsLocation => ( is => 'rw', default => sub { [] } );
