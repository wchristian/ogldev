...
has $_ => ( is => 'rw' )
  for
  qw( WVPLocation LightWVPLocation WorldMatrixLocation colorMapLocation shadowMapLocation normalMapLocation eyeWorldPosLocation matSpecularIntensityLocation matSpecularPowerLocation numPointLightsLocation numSpotLightsLocation);
...
sub Init {
    ...
    $self->LightWVPLocation( $self->GetUniformLocation( "gLightWVP" ) );
    $self->WorldMatrixLocation( $self->GetUniformLocation( "gWorld" ) );
    $self->colorMapLocation( $self->GetUniformLocation( "gColorMap" ) );
    $self->shadowMapLocation( $self->GetUniformLocation( "gShadowMap" ) );
    $self->normalMapLocation( $self->GetUniformLocation( "gNormalMap" ) );
    ...
    if (   $self->dirLightLocation->{AmbientIntensity} == 0xFFFFFFFF
        ...
        || $self->LightWVPLocation == 0xFFFFFFFF
        || $self->WorldMatrixLocation == 0xFFFFFFFF
        || $self->colorMapLocation == 0xFFFFFFFF
        || $self->shadowMapLocation == 0xFFFFFFFF
        || $self->normalMapLocation == 0xFFFFFFFF
...
sub SetLightWVP {
    my ( $self, $LightWVP ) = @_;
    glUniformMatrix4fvARB_s( $self->LightWVPLocation, 1, GL_TRUE, $LightWVP->get_dataref );
}
...
sub SetShadowMapTextureUnit {
    my ( $self, $TextureUnit ) = @_;
    glUniform1iARB( $self->shadowMapLocation, $TextureUnit );
}

sub SetNormalMapTextureUnit {
    my ( $self, $TextureUnit ) = @_;
    glUniform1iARB( $self->normalMapLocation, $TextureUnit );
}
