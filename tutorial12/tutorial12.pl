...
sub RenderSceneCB {
    ...
    my $p = pipeline->new(
        Rotate   => [ 1, $Scale, 0 ],
        WorldPos => [ 0, 0,      5 ],
        PerspectiveProj => { FOV => 30, Width => WINDOW_WIDTH, Height => WINDOW_HEIGHT, zNear => 1, zFar => 100 },
    );

    glUniformMatrix4fvARB_s( $gWorldLocation, 1, GL_TRUE, $p->GetWPTrans->get_dataref );
