...
has $_ => ( is => 'rw' )
  for
  qw( WVPLocation LightWVPLocation colorTextureLocation shadowMapLocation WorldMatrixLocation eyeWorldPosLocation matSpecularIntensityLocation matSpecularPowerLocation numPointLightsLocation numSpotLightsLocation);
...
sub Init {
    ...
    if ( !$self->AddShader( GL_VERTEX_SHADER, "lighting.vs" ) ) {
        return;
    }

    if ( !$self->AddShader( GL_FRAGMENT_SHADER, "lighting.fs" ) ) {
        return;
    }
